...
export function updateCountdown(targetDate: string) {
  const days = q('#days') as HTMLElement | null;
  const hours = q('#hours') as HTMLElement | null;
  const mins = q('#minutes') as HTMLElement | null;
  const secs = q('#seconds') as HTMLElement | null;
